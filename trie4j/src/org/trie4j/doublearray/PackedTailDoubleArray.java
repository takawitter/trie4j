...
public class PackedTailDoubleArray extends AbstractTermIdTrie implements TermIdTrie, Externalizable{
	public static interface TermNodeListener{
		void listen(Node node, int nodeIndex);
	}

	public PackedTailDoubleArray(){
	}

	public PackedTailDoubleArray(Trie orig){
		this(orig, new SuffixTrieTailBuilder());
	}

	public PackedTailDoubleArray(Trie orig, TailBuilder tb){
	...
	public PackedTailDoubleArray(Trie orig, TailBuilder tb, TermNodeListener listener){
		size = orig.size();
		int as = size;
		if(as <= 1) as = 2;
		base = new int[as];
		...
		Map<Integer, Integer> tailIndices = new TreeMap<Integer, Integer>();
		FastBitSet termBs = new FastBitSet(65536);
		FastBitSet tailBs = new FastBitSet(65536);
		build(orig.getRoot(), 0, tb, termBs, tailBs, tailIndices, listener);
		term = new Rank1OnlySuccinctBitVector(termBs.getBytes(), termBs.size());
		tail = new Rank1OnlySuccinctBitVector(tailBs.getBytes(), tailBs.size());
		int n = tailIndices.size();
		tailIndex = new int[n];
		Iterator<Integer> it = tailIndices.values().iterator();
		for(int i = 0; i < n; i++){
			tailIndex[i] = it.next();
		}
		tails = tb.getTails();
	...
	private class TailDoubleArrayNode implements TermIdNode{
		...
		public char[] getLetters() {
			...
			if(tail.isOne(nodeId)){
				TailUtil.appendChars(tails, tailIndex[tail.rank1(nodeId) - 1], ret);
			}
	...
	public int getTermId(String text) {
		...
		for(int i = 0; i < n; i++){
			...
			nodeIndex = next;
			if(!tail.isOne(nodeIndex)) continue;
			it.setIndex(tailIndex[tail.rank1(nodeIndex) - 1]);
			char c;
	...
	public Iterable<Pair<String, Integer>> commonPrefixSearchWithTermId(String query) {
		...
		for(; ci < charsLen; ci++){
			...
			ni = next;
			if(tail.isOne(ni)){
				it.setIndex(tailIndex[tail.rank1(ni) - 1]);
				char c;
				while((c = it.getNext()) != '\0'){
					ci++;
	...
	public void readExternal(ObjectInput in) throws IOException,
		...
		try{
			tail = (SuccinctBitVector)in.readObject();
			tailIndex = (int[])in.readObject();
	...
	public void dump(Writer w){
		...
		try{
			...
			for(int i = 0; i < 16; i++){
				if(tail.isOne(i)){
					writer.print(String.format("%3d|", tailIndex[tail.rank1(i) - 1]));
				} else{
					writer.print("N/A|");
				}
			}
			...
			writer.println();
			writer.println("tail count: " + tailIndex.length);
			writer.println();
			writer.print("tails: [");
	...
	private void build(Node node, int nodeIndex, TailBuilder tailb,
			FastBitSet termBs, FastBitSet tailBs, Map<Integer, Integer> tailIndices,
			TermNodeListener listener){
		// letters
		char[] letters = node.getLetters();
		if(letters.length > 1){
			tailBs.set(nodeIndex);
			tailIndices.put(nodeIndex, tailb.insert(letters, 1, letters.length - 1));
		} else{
			tailBs.unsetIfLE(nodeIndex);
		}
		if(node.isTerminate()){
			termBs.set(nodeIndex);
			listener.listen(node, nodeIndex);
		} else if(termBs.size() <= nodeIndex){
			termBs.unsetIfLE(nodeIndex);
		}
		...
		for(Map.Entry<Integer, List<Pair<Node, Integer>>> e : nodes.entrySet()){
			for(Pair<Node, Integer> e2 : e.getValue()){
				build(e2.getFirst(), e2.getSecond() + offset, tailb,
						termBs, tailBs, tailIndices, listener);
			}
		}
//*/
	}
...
//*/
	...
	private SuccinctBitVector tail;
