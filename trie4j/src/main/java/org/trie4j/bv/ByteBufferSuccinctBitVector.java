...
public class ByteBufferSuccinctBitVector
implements Externalizable, SuccinctBitVector{
	public ByteBufferSuccinctBitVector(){
		this(16);
	}

	public ByteBufferSuccinctBitVector(int initialCapacity){
		this.bytes = ByteBuffer.allocateDirect(bytesSize(initialCapacity));
	...
	public ByteBufferSuccinctBitVector(byte[] bytes, int bitsSize){
		this.bytes = ByteBuffer.wrap(bytes, 0, bytesSize(bitsSize));
		this.size = bitsSize;
...
*/	}
	...
	public String toString() {
		...
		for(int i = 0; i < n; i++){
			b.append((bytes.get(i / 8) & (0x80 >> (i % 8))) != 0 ? "1" : "0");
		}
		return b.toString();
	}
	...
	public boolean isZero(int pos){
		return (bytes.get(pos / 8) & BITS[pos % 8]) == 0;
	}
	...
	public void trimToSize(){
		...
		bytes.limit(Math.min(bytes.limit(), vectorSize));
	...
	public void append1(){
		...
		int r = size % 8;
		bytes.put(i, (byte)(bytes.get(i) | BITS[r]));
		size++;
	}
	...
	public int rank0(int pos){
		...
		for(int i = (cn * (CACHE_WIDTH / 8)); i < n; i++){
			ret += BITCOUNTS0[bytes.get(i) & 0xff];
		}
		return ret + BITCOUNTS0[(bytes.get(n) | ~MASKS[pos % 8]) & 0xff];
	}
	...
	public int rank1(int pos){
		...
		int n = pos / 8;
		for(int i = (cn * (CACHE_WIDTH / 8)); i < n; i++){
			ret += BITCOUNTS1[bytes.get(i) & 0xff];
		}
		return ret + BITCOUNTS1[bytes.get(n) & MASKS[pos % 8]];
	}
	...
	public int select0_(int count){
		...
		if(count > 0){
			for(; i < bytes.limit(); i++){
				...
				int c = BITCOUNTS0[bytes.get(i) & 0xff];
				if(count <= c){
...
//		int n = containerBytesCount(size);
		for(int i = (m + 1) * CACHE_WIDTH / 8; i < bytes.limit(); i++){
			int bits = bytes.get(i) & 0xff;
	...
	public int select1(int count){
		for(int i = 0; i < bytes.limit(); i++){
			if(i * 8 >= size) return -1;
			int c = BITCOUNTS1[bytes.get(i) & 0xff];
			if(count <= c){
				int v = bytes.get(i) & 0xff;
				for(int j = 0; j < 8; j++){
					if(i * 8 + j >= size) return -1;
	...
	public int next0(int pos){
		...
		int s = pos % 8;
		if(s != 0){
			for(byte b : BITPOS0[bytes.get(i) & 0xff]){
				if(s <= b) return i * 8 + b;
			}
			i++;
		}
		int n = size / 8 + 1;
		for(; i < n; i++){
			byte[] poss = BITPOS0[bytes.get(i) & 0xff];
			if(poss.length > 0){
				return poss[0] + i * 8;
			}
		}
	...
	throws ClassNotFoundException, IOException{
		...
		in.readFully(tmp, 0, vectorSize);
		bytes = ByteBuffer.wrap(tmp);
		int size = in.readInt();
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		...
		trimToSize();
		out.writeInt(bytes.limit());
		if(bytes.hasArray()) {
			out.write(bytes.array());
		} else {
			var buff = new byte[bytes.limit()];
			bytes.get(0, buff);
			out.write(buff);
		}
	...
	private void extend(){
		int vectorSize = (int)(bytes.limit() * 1.2) + 1;
		var newBytes = ByteBuffer.allocateDirect(vectorSize);
		newBytes.put(bytes);
		newBytes.position(0);
		bytes = newBytes;
